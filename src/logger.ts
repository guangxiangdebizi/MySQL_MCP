...
// 工具函数：记录SQL操作
export function logSqlOperation(operation: string, query: string, params?: any[], duration?: number, error?: Error) {
  const logData = {
    operation,
...
// 工具函数：记录连接操作
export function logConnection(action: 'connect' | 'disconnect', config?: any, error?: Error) {
  const logData = {
    action,
    host: config?.host,
    port: config?.port,
