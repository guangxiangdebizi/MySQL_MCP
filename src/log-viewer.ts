...
class LogViewer {
   ...
   * 按时间范围过滤日志
   */
  filterByTimeRange(logs: LogEntry[], startTime: string, endTime: string): LogEntry[] {
   ...
   * 获取最近的错误
   */
  getRecentErrors(logs: LogEntry[], count: number = 10): LogEntry[] {
    return logs
   ...
   * 分析数据库操作
   */
  analyzeDatabaseOperations(logs: LogEntry[]): void {
    const dbOps = logs.filter(log => 
      log.message.includes('SQL操作') || 
