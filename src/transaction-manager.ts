...
export interface TransactionOperation {
  id: string;
  timestamp: Date;
  type: 'INSERT' | 'UPDATE' | 'DELETE' | 'CREATE_TABLE' | 'DROP_TABLE';
  tableName: string;
  description: string;
  query: string;
  params?: any[];
...
export class TransactionManager {
  ...
  private operationHistory: TransactionOperation[] = [];
   ...
   * 记录操作到历史
   */
  recordOperation(operation: Omit<TransactionOperation, 'id' | 'timestamp'>): void {
   ...
   * 获取操作历史
   */
  getOperationHistory(): TransactionOperation[] {
   ...
   * 回滚到指定操作步骤
   */
  async rollbackToStep(stepNumber: number, executeRollback: (query: string, params?: any[]) => Promise<any>): Promise<string> {
    if (!this.isTransactionActive) {
      throw new Error("没有活跃的事务");
    }
   ...
   * 完全回滚事务
   */
  async fullRollback(executeRollback: (query: string, params?: any[]) => Promise<any>): Promise<string> {
    if (!this.isTransactionActive) {
      throw new Error("没有活跃的事务");
    }
   ...
   * 提交事务
   */
  async commitTransaction(executeCommit: () => Promise<any>): Promise<string> {
    if (!this.isTransactionActive) {
      throw new Error("没有活跃的事务");
    }
    ...
    const operationsSummary = this.operationHistory.map((op, index) => 
      `${index + 1}. ${op.type} - ${op.description} (${op.tableName})`
    ).join('\n');
